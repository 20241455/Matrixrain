...
namespace MatrixPorted {
	...
	class Program {
		...
		static string ColorizeText<T>(T text, int r, int g) =>
			"\x1b[38;2;" + r + ";" + g + ";0m" + text;
		...
		static void UpdateScreen(Object? data, System.Timers.ElapsedEventArgs? _)
		{
			...
			if (cmdline.Length != 0) {
				Console.SetCursorPosition(0, 0);
				Console.Write(ColorizeText((cmdline + "█").PadRight(Console.WindowWidth), 127, 64));
				y++;
			}
		...
		static void SetupMask(string message)
		{
			...
			int fixedy = Math.Min(Console.WindowHeight / 4, yskulloffset - 8) - newline_amount * 8 - 4;
					...
					if (cmdline != "") {
						switch (cmdline) {
							...
							default:
								...
								} else if (Font.SKULL.GetLength(0) + 8 > Console.WindowHeight) {
